...
def jobb_feltet_hatter(angle, speed = 400, timeout=None): #létrehozunk egy jobb_feltet nevű függvényt, amiben paraméterként benne van a fok és a sebesség és ez a jobb feltét motorra vonatkozik
    if timeout != None:
        timeout_watch = StopWatch()
        timeout_watch.reset()
        timeout_watch.resume()
        feltet_jobb.run_angle(speed, angle, wait=False) #a feltet_balt lefutatjuk fokban, itt paraméterként a speed és az angle van megadva beszorozva a bal_atettel
        while abs(feltet_jobb.angle()-(angle)) > 1:
            if timeout_watch.time() >= timeout: break
    else:
        feltet_jobb.run_angle(speed, angle) #a feltet_balt lefutatjuk fokban, itt paraméterként a speed és az angle van megadva beszorozva a bal_atettel

 
def bal_feltet_hatter(angle, speed = 400, timeout = None): #létrehozunk egy bal_feltet nevű függvényt, amiben paraméterként benne van a fok és a sebesség és ez a bal feltét motorra vonatkozik
    if timeout != None:
        timeout_watch = StopWatch()
        timeout_watch.reset()
        timeout_watch.resume()
        feltet_bal.run_angle(speed, angle, wait=False) #a feltet_balt lefutatjuk fokban, itt paraméterként a speed és az angle van megadva beszorozva a bal_atettel
...
def jobb_feltet(angle, speed = 400): #létrehozunk egy jobb_feltet_hatter nevű függvényt, amiben paraméterként benne van a fok és a sebesség és ez a jobb feltét motorra vonatkozik és ez mozgás közben is működik
    feltet_jobb.run_angle(speed, angle, wait=True) #a feltet_jobbat lefutatjuk fokban, itt paraméterként a speed és az angle van megadva beszorozva a jobb_atettel és itt a nem vár hanem lefut hatterben
 
def bal_feltet(angle, speed = 400): #létrehozunk egy bal_feltet_hatter nevű függvényt, amiben paraméterként benne van a fok és a sebesség és ez a bal feltét motorra vonatkozik és ez mozgás közben
    feltet_bal.run_angle(speed, angle, wait=True) #a feltet_balt lefutatjuk fokban, itt paraméterként a speed és az angle van megadva beszorozva a bal_atettel és itt a nem vár hanem lefut hatterben
